 ...
 *
 */
public class PackageInfoSheetV2d3 extends PackageInfoSheet {
	...
	int PRIMARY_PURPOSE_COL = COMMENT_COL + 1;
	int RELEASE_DATE_COL = PRIMARY_PURPOSE_COL + 1;
	int BUILT_DATE_COL = RELEASE_DATE_COL + 1;
	int VALID_UNTIL_COL = BUILT_DATE_COL + 1;
	int USER_DEFINED_COL = VALID_UNTIL_COL + 1;
	...
	static final String[] HEADER_TITLES = new String[] {"Package Name", "SPDX Identifier", "Package Version", 
		...
		"Attribution Text", "Files Analyzed", "Comments", "Primary Purpose", 
		"Release Date", "Built Date", "Valid Until Date", "User Defined Columns..."};
	 ...
	 * @param copyManager 
	 */
	public PackageInfoSheetV2d3(Workbook workbook, String sheetName, String version, 
...
//				}
	...
	public void add(SpdxPackage pkgInfo) throws InvalidSPDXAnalysisException {
		...
		Optional<Purpose> purpose = pkgInfo.getPrimaryPurpose();
		if (purpose.isPresent()) {
			row.createCell(PRIMARY_PURPOSE_COL).setCellValue(purpose.get().toString());
		}
		Optional<String> builtDate = pkgInfo.getBuiltDate();
		if (builtDate.isPresent()) {
			Cell cell = row.createCell(BUILT_DATE_COL);
			try {
				cell.setCellValue(DATE_FORMAT.get().parse(builtDate.get()));
		...
		if (releaseDate.isPresent()) {
			...
			try {
				cell.setCellValue(DATE_FORMAT.get().parse(releaseDate.get()));
			} catch (ParseException e) {
				throw(new SpreadsheetException("Invalid created date - unable to parse"));
			}
			cell.setCellStyle(dateStyle);
		...
		if (validUntilDate.isPresent()) {
			...
			try {
				cell.setCellValue(DATE_FORMAT.get().parse(validUntilDate.get()));
			} catch (ParseException e) {
				throw(new SpreadsheetException("Invalid created date - unable to parse"));
			}
			cell.setCellStyle(dateStyle);
	...
	private SpdxPackage getPackage(int rowNum) throws SpreadsheetException, DefaultStoreNotInitializedException {
		...
		String declaredCopyright = null;
		Cell declaredCopyrightCell = row.getCell(DECLARED_COPYRIGHT_COL);
		if (declaredCopyrightCell != null && !declaredCopyrightCell.getStringCellValue().isEmpty()) {
			declaredCopyright = declaredCopyrightCell.getStringCellValue();
		}
		
		AnyLicenseInfo declaredLicenses = null;
		...
		Cell primaryPurposeCell = row.getCell(PRIMARY_PURPOSE_COL);
		if (Objects.nonNull(primaryPurposeCell) && !primaryPurposeCell.getStringCellValue().isEmpty()) {
			try {
				retval.setPrimaryPurpose(Purpose.valueOf(primaryPurposeCell.getStringCellValue().trim()));
			} catch (Exception ex) {
		...
		if (Objects.nonNull(releaseDateCell)) {
			retval.setReleaseDate(DATE_FORMAT.get().format(releaseDateCell.getDateCellValue()));
		}
		Cell builtDateCell = row.getCell(BUILT_DATE_COL);
		if (Objects.nonNull(builtDateCell)) {
			retval.setBuiltDate(DATE_FORMAT.get().format(builtDateCell.getDateCellValue()));
		}
		Cell validUntilDateCell = row.getCell(VALID_UNTIL_COL);
		if (Objects.nonNull(validUntilDateCell)) {
			retval.setValidUntilDate(DATE_FORMAT.get().format(validUntilDateCell.getDateCellValue()));
		}
		try {
			return retval.build();
		} catch (InvalidSPDXAnalysisException e) {
